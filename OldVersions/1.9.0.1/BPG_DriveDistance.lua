...
function BPG_DriveDistance:onPostLoad(savegame)
	...
	spec.textSize = g_currentMission.inGameMenu.hud.speedMeter.speedUnitTextSize*0.85
...
function BPG_DriveDistance:onDraw(isActiveForInput, isActiveForInputIgnoreSelection, isSelected)
	...
	if self.spec_enterable.isEntered then
		...
		renderText(offsetX,0.014775,spec.textSize,"Trip "..string.format("%.2f", tostring(math.floor(spec.driveDistanceToday*spec.distanceFactor)/100)).." "..spec.distanceText)
		renderText(offsetX,totalOffsetY,spec.textSize,string.format("%.2f", tostring(math.floor(spec.driveDistanceOverall*spec.distanceFactor)/100)).." "..spec.distanceText)
